...
async function sendEmailToDoctor(doctorEmail, subject, templateName, templateData = {}) {
  try {
    // Validate email
    ...
    // Fetch user details
    const user = await doctorModel.findOne({ email: doctorEmail });
    console.log(user)
    if (!user) {
      throw new NotFoundError(`User with email ${doctorEmail} not found`);
    }
    ...
    // Send email
    const response = await sendMailToUser({ to: doctorEmail, subject, html });
    console.log(`${subject} Email sent:`, response);
  } catch (error) {
    if (error instanceof AppError) {
      console.error(`AppError: ${error.message}`);
...
export { sendEmailToDoctor, sendEmail };
